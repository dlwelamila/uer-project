...
const HEALTH_TABS = [
  { href: '/health/code-currency', label: 'Code Currency' },
  { href: '/health/connectivity', label: 'Connectivity' },
  { href: '/health/fco-tse', label: 'FCO & TSE' },
  { href: '/health/advisories', label: 'Advisories' },
  { href: '/health/capacity-review', label: 'Capacity Review' },
] as const

export default function HealthLayout({ children }: { children: React.ReactNode }) {
  return (
    <IncidentsFiltersProvider>
      <HealthLayoutShell>{children}</HealthLayoutShell>
    </IncidentsFiltersProvider>
  )
}

function HealthLayoutShell({ children }: { children: React.ReactNode }) {
  const {
    organizations,
  ...
  if (!organizations.length) {
    return (
      <div className="rounded-lg bg-white p-4 shadow">
        <h1 className="mb-2 text-lg font-semibold">Health &amp; Risk Analysis</h1>
        <p className="text-sm text-slate-700">
          No customers found. Add organizations before reviewing health analysis.
        </p>
      </div>
    )
  }
      ...
      <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            value={reportType}
            onChange={(event) =>
              setReportType(event.target.value as (typeof REPORT_TYPES)[number]['value'])
            }
          >
